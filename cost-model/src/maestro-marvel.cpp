...
maestro_api->AddTensor("input", maestro::DFA::TensorClass::InputTensor,
maestro::DataClass::Input, input_correlated_vars);
maestro_api->AddTensor("weight", maestro::DFA::TensorClass::InputTensor,
maestro::DataClass::Weight, weight_correlated_vars);
maestro_api->AddTensor("output", maestro::DFA::TensorClass::OutputTensor,
maestro::DataClass::Output, output_correlated_vars);
...
false); maestro_api->AddNoC(option.bw, option.hops, option.hop_latency,
option.mc, false); maestro_api->AddNoC(option.bw, option.hops,
option.hop_latency, option.mc, false); maestro_api->AddNoC(option.bw,
option.hops, option.hop_latency, option.mc, false);

std::shared_ptr<std::list<std::shared_ptr<std::pair<std::string, std::string>>>>
overlap_dim_list =
std::make_shared<std::list<std::shared_ptr<std::pair<std::string,
std::string>>>>(); auto output_column_overlap =
std::make_shared<std::pair<std::string, std::string>>("X", "S"); auto
output_row_overlap = std::make_shared<std::pair<std::string, std::string>>("Y",
"R"); overlap_dim_list->push_back(output_column_overlap);
...
maestro_get_costs(int num_pes, int l1_buffer_size, int l2_buffer_size,
                  int noc_bw_value,
                  std::shared_ptr<maestro::DFA::Layer> layer) {
  ...
  auto config = std::make_shared<maestro::ConfigurationV2>(
      option.dfsl_file_name, noc_bw, noc_latency, noc_multcast, option.np,
      option.num_simd_lanes, option.bw, option.l1_size, option.l2_size);
